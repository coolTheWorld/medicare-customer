...
<head>
    ...
    <script type="text/javascript" src="${path}/evolution/lib/layui/layui.js" charset="utf-8"></script>
    ...
    <![endif]-->
    <script type="text/javascript" src="${path}/evolution/js/html5.min.js"></script>
    <script type="text/javascript" src="${path}/evolution/js/respond.min.js"></script>
</head>
<body>
        ...
        <div class="layui-col-md6">
            <div class="layui-card contont">
                ...
                <div class="layui-card-body help-p">
                    ...
                    <!-- <p >文字</p> -->
                    <br/>
                    <h2>3、选中需要确认的数据的日期</h2>
                    <!-- <p >文字</p> -->
                    <br/>
                    <h2>4、在虚线框中选择对应的记录(例如入库记录、出库记录、库存记录、销售记录)</h2>
                    ...
                    <form class="layui-form layui-form-pane" action="/dataConfirm/confirm">
                        <div class="layui-col-md12 item-magrin">
                            <input id = "lay-date-confirm" name = "recordDate" class="layui-input" type="text" lay-verify="required"  style="width: 20%;" placeholder = "点击此处选择日期"/>
                        </div>
                        <div id="div-border" class="layui-col-md12 item-magrin div-center">
                            ...
                            <c:if test="${customer.cusFlag eq '201' or customer.cusFlag eq '301'}">
                                <input type="checkbox" value="1" name="recordPrescribe" title="处方记录"
                                       lay-skin="primary"/>
                            </c:if>
                            <c:if test="${customer.cusFlag eq '201' or customer.cusFlag eq '301'}">
                                <input type="checkbox" value="1" name="recordClinicrecords" title="门诊记录"
                                       lay-skin="primary"/>
                            </c:if>
                            <c:if test="${customer.cusFlag eq '301'}">
                                <input type="checkbox" value="1" name="recordHospitalized" title="住院记录"
                                       lay-skin="primary"/>
                            </c:if>
                            <c:if test="${customer.cusFlag eq '301'}">
                                <input type="checkbox" value="1" name="recordDischarged" title="出院记录"
                                       lay-skin="primary"/>
                            </c:if>
                        </div>
                        <div class="layui-col-md12 item-magrin div-center">
                            <input type="submit" id="btn-submit" class="layui-btn" lay-submit lay-filter="confirm"
                                   value="数据确认"/>
                        </div>
                    </form>
                    <div class="layui-col-md12 item-magrin">
...
<script src="${path}/evolution/js/jquery.min.js" type="text/javascript"></script>
    ...
    $(function () {
        ...
        layui.use(['form', 'layer', 'table','laydate'], function () {
            ...
            form.on('submit(confirm)', function (data) {
                console.log(data.field.recordDate);
                var jsonData = {
                ...
                $.ajax({
                    ...
                    success: function (ajaxData) {
                        if (ajaxData.flag == 1) {
                            layer.msg(ajaxData.mes, {offset: 't', icon: 1, time: 7000, area: '200px'});
                            table.reload('confirmData', {
                                url: '/dataConfirm/findByCustomer',
                                where: {
                                    date:data.field.recordDate
                                }
                            });
                        } else if (ajaxData.flag == 0) {
                            layer.msg(ajaxData.mes, {offset: 't', icon: 7, time: 7000, area: '250px'});
                        }
                    },
