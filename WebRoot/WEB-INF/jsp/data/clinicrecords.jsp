		...
		<script type="text/javascript">
			...
			function deleteRepeat(type){
				...
				$.ajax({
					type: "POST",
			        url: "deleteRepeat",
			        data:{type:type,cusId:cusId,name:'诊断记录'},
			        success: function(data) {
			        	$("#clinicrecords").datagrid('reload');
			        	$.messager.show({
							title: "系统提示",
			                msg: data.info
						});
				...
				<form id="searchForm" onkeydown="if(event.keyCode==13){ queryForm();}">
					<table>
						<tr>
							...
							<td><a class="easyui-linkbutton" href="javascript:void(0);" iconCls="icon-cancel" onclick="deleteRepeat('clinicrecords');">删除重复数据</a></td>
						</tr>
					</table>
				</form>
			</div>
