		...
		<script type="text/javascript">
			...
			function deleteRepeat(type){
				...
				$.ajax({
					type: "POST",
			        url: "deleteRepeat",
			        data:{type:type,cusId:cusId,name:'入库信息'},
			        success: function(data) {
			        	$("#warehouseitem").datagrid('reload');
			        	$.messager.show({
							title: "系统提示",
			                msg: data.info
						});
			...
			<div data-options="region:'north',title:'高级查询'" style="height: 65px; background: #F4F4F4;">
				<form id="searchForm" onkeydown="if(event.keyCode==13){ queryForm();}">
					<table>
						<tr>
							<th>入库开始日期</th>
							<td><input class="easyui-datebox" id="startDate" data-options="editable:false" name="startDate" style="width:100px" /></td><!-- editable="false"来禁止用户手动输入，easyui-datetimebox：带时分秒 -->
							<th>入库结束日期</th>
							...
							<td><a class="easyui-linkbutton" href="javascript:void(0);" iconCls="icon-cancel" onclick="deleteRepeat('warehouseitem');">删除重复数据</a></td>
						</tr>
					</table>
				</form>
			</div>
