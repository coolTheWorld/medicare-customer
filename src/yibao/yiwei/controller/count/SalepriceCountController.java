...
@Controller
public class SalepriceCountController {
	 ...
	 */
	@RequestMapping("/toListDrugSalepriceChart")
	public String toListDrugSalepriceChart(String cusFlag, HttpServletRequest request) throws ParseException {
		request.setAttribute("cusFlag", cusFlag);
		return "/salesitem/listDrugSalepriceChart";
	}
	 ...
	 */
	@RequestMapping("/toDrugSalepriceChart")
	public String toDrugSalepriceChart(String cusFlag, String drugName, HttpServletRequest request) throws UnsupportedEncodingException {
		request.setAttribute("cusFlag", cusFlag);
		request.getSession().setAttribute("drugName", drugName);
		return "/salesitem/drugSalepriceCountChart";
	}
	 ...
	 * @param request
	 * @param session 权限控制
	 * @param response 药品销售价柱形图Json
	 * @throws ParseException
	 * @throws IOException
	 */
	@RequestMapping("/getDrugSalepriceCountChart")
	public void getDrugSalepriceCountChart(String cusFlag, String drugName, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws ParseException, IOException {
		...
		if (arrList.size() > 0) {
			...
			// 定义药品(药品销售明细表)统计销售价list(无序)
			List disorderTotals = new ArrayList();
			Object total;// 定义临时变量
			for (int i = 0; i < realarrList.size(); i++) {
				String hql2 = "select s.drugSalesprice from Salesitem s where s.cusId=?0 and s.drugCode=?1 and s.drugPicktime between ?2 and ?3 order by s.drugSalesprice desc ";
				// 根据cusIds集合,药品名称集合查询(药品销售明细表)计算统计销售价
			...
			for (Map.Entry<String, Double> mapping : list) {
				...
				Double order = mapping.getValue();// 销售价
				totals.add(order);
				num++;
				if (num > 10)
					break;// 超过十家定点跳出
		...
		json.accumulate("totals", totals);// 销售价
